...
import {
    ...
    ObjectLiteralExpression, ArrayLiteralExpression, TextChangeRange, LabeledStatement, Symbol
} from "./types";
import {INode as Node} from "./types";
import {
    lastOrUndefined, hasProperty, forEach, createFileDiagnostic, indexOf, combinePaths,Debug,
    getDirectoryPath, isRootedDiskPath, getBaseFileName, concatenate, filter, sortAndDeduplicateDiagnostics,
    removeFileExtension, fileExtensionIs, getNormalizedAbsolutePath, createCompilerDiagnostic, reduceProperties,
    getRelativePathToDirectoryOrUrl, toPath, supportedJavascriptExtensions, reduceLeft, isArray
} from "./core";
import {
    createScanner, skipTrivia, getLeadingCommentRanges, getTrailingCommentRanges,
    getLineAndCharacterOfPosition, getLineStarts, computeLineAndCharacterOfPosition, computeLineStarts, isWhiteSpace
} from "./scanner";
import {Diagnostics} from "./diagnostics";
import {sys} from "./sys";
import {forEachChild, createNode} from "./parser";
import {getModuleInstanceState, ModuleInstanceState} from "./binder";
...
export function isDynamicName(name:DeclarationName):boolean {
    return name.kind === SyntaxKind.ComputedPropertyName && !isStringOrNumericLiteral((<ComputedPropertyName>name).expression.kind) && !isWellKnownSymbolSyntactically((<ComputedPropertyName>name).expression);
}
...
export function cloneEntityName(node:EntityName, parent?:Node):EntityName {
    ...
    if (isQualifiedName(clone)) {
        ...
        clone.right = <any>cloneNode(right, right, right.flags, parent);
    }
...
export function modifierToFlag(token:SyntaxKind):NodeFlags {
    switch (token) {
        ...
        case SyntaxKind.DefaultKeyword:
            return NodeFlags.Default;
        case SyntaxKind.AsyncKeyword:
            return NodeFlags.Async;
    }
    return 0;
}
