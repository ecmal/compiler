...
import {
    ModuleDeclaration, VariableStatement, SyntaxKind, NodeFlags, PropertyAccessExpression,
    BinaryExpression, Identifier, SymbolFlags, CompilerOptions, SourceFile, INode as Node, Map, Declaration,
    LiteralExpression, ComputedPropertyName, ExportAssignment, SpecialPropertyAssignmentKind, JSDocFunctionType,
    SymbolTable, FunctionLikeDeclaration, WhileStatement, DoStatement, ForStatement, ForInStatement, ForOfStatement,
    IfStatement, ReturnStatement, ThrowStatement, BreakOrContinueStatement, TryStatement, SwitchStatement, CaseBlock,
    LabeledStatement, Block, SignatureDeclaration, ObjectLiteralExpression, CatchClause, DeleteExpression,
    PostfixUnaryExpression, PrefixUnaryExpression, WithStatement, DiagnosticMessage, ModuleBlock, NodeArray, Statement,
    ExpressionStatement, TypePredicateNode, ParameterDeclaration, VariableDeclaration, BindingElement,
    PropertyDeclaration, MethodDeclaration, FunctionDeclaration, TypeLiteralNode, FunctionExpression, CallExpression,
    ClassLikeDeclaration, EnumDeclaration, ImportClause, ExportDeclaration, Symbol
} from "./types";
import {Diagnostics} from "./diagnostics";
import {
    forEach, objectAllocator, Debug, indexOf, hasProperty, createFileDiagnostic, removeFileExtension,
    lastOrUndefined
} from "./core";
import {
    getCombinedNodeFlags, isInAmbientContext, isStatement, isConstEnumDeclaration,
    isGlobalScopeAugmentation, isAmbientModule, isStringOrNumericLiteral, isWellKnownSymbolSyntactically,
    getPropertyNameForKnownSymbolName, getSpecialPropertyAssignmentKind, isJSDocConstructSignature,
    declarationNameToString, hasDynamicName, createDiagnosticForNode, isFunctionLikeKind, isInJavaScriptFile,
    nodeIsPresent, isFunctionLike, isExternalModule, getErrorSpanForNode, isIdentifierName, getContainingClass,
    isLeftHandSideExpression, isAssignmentOperator, getSpanOfTokenAtPosition, isPrologueDirective,
    getTextOfNodeFromSourceText, isObjectLiteralMethod, isRequireCall, getClassExtendsHeritageClauseElement,
    isDeclarationFile, nodeIsDecorated, isConst, isBindingPattern, isBlockOrCatchScoped, isParameterDeclaration,
    isParameterPropertyDeclaration, isAsyncFunctionLike
} from "./utilities";
...
function createBinder():(file:SourceFile, options:CompilerOptions) => void {
    ...
    function bindSourceFile(f:SourceFile, opts:CompilerOptions) {
        ...
        Symbol = <any>objectAllocator.getSymbolConstructor();
    ...
    function checkStrictModeIdentifier(node:Identifier) {
        if (inStrictMode &&
            ...
            node.originalKeywordKind <= SyntaxKind.LastFutureReservedWord && !isIdentifierName(node)) {
            ...
            case Reachability.Unreachable:
                ...
                if (reportError) {
                    ...
                    const reportUnreachableCode =
                        !options.allowUnreachableCode && !isInAmbientContext(node) &&
                        (
