/* @internal */
import {
    getDirectoryPath, isRootedDiskPath, combinePaths, normalizePath, getSupportedExtensions,
    getRootLength, forEach, fileExtensionIs, normalizeSlashes, getBaseFileName, supportedTypeScriptExtensions,
    createCompilerDiagnostic, hasProperty, sortAndDeduplicateDiagnostics, createFileMap, getNormalizedAbsolutePath,
    directorySeparator, copyMap, concatenate, map, toPath, addRange, Debug, createFileDiagnostic, memoize,
    getNormalizedPathComponents, getNormalizedPathFromPathComponents
} from "./core";
import {
    CompilerOptions, ModuleResolutionHost, ResolvedModuleWithFailedLookupLocations, ModuleKind,
    ModuleResolutionKind, CharacterCodes, CompilerHost, Map, ScriptTarget, SourceFile, Program, CancellationToken,
    Diagnostic, DiagnosticMessageChain, TypeChecker, ResolvedModule, WriteFileCallback, EmitResult,
    OperationCanceledException, SyntaxKind, HeritageClause, ClassDeclaration, FunctionLikeDeclaration,
    VariableStatement, VariableDeclaration, CallExpression, ParameterDeclaration, TypeAssertion, NodeArray,
    TypeParameterDeclaration, TypeNode, ModifiersArray, FileReference, LiteralExpression, INode as Node, Path,
    ExportAssignment, ModuleBlock, ModuleDeclaration, NodeFlags, StringLiteral, DiagnosticMessage, FileMap
} from "./types";
import {
    getEmitModuleKind, getNewLineCharacter, createDiagnosticCollection, arrayIsEqualTo,
    getResolvedModule, EmitHost, isSourceFileJavaScript, createDiagnosticForNode, isDeclarationFile, isExternalModule,
    getExternalModuleName, getDefaultLibFileName, isExternalModuleNameRelative, isAmbientModule, isRequireCall,
    setResolvedModule, getTokenPosOfNode, getErrorSpanForNode, forEachExpectedEmitFile
} from "./utilities";
import {sys} from "./sys";
import {Diagnostics} from "./diagnostics";
import {createSourceFile, forEachChild} from "./parser";
import {createTypeChecker} from "./checker";
import {emitFiles} from "./emitter";
import {getDeclarationDiagnostics as getDeclarationDiagnosticsEmit} from "./declarationEmitter";
import {tokenToString, skipTrivia, isIdentifier} from "./scanner";
...
function loadModuleFromNodeModules(moduleName:string, directory:string, host:ModuleResolutionHost):ResolvedModuleWithFailedLookupLocations {
    ...
    while (true) {
        ...
        if (baseName !== "node_modules") {
            ...
            if (result) {
                return {
                    resolvedModule: {resolvedFileName: result, isExternalLibraryImport: true},
    ...
    function getDeclarationDiagnosticsForFile(sourceFile:SourceFile, cancellationToken:CancellationToken):Diagnostic[] {
        return runWithCancellationToken(() => {
            if (!isDeclarationFile(sourceFile)) {
                ...
                return getDeclarationDiagnosticsEmit(getEmitHost(writeFile), resolver, sourceFile);
            }
        });
    }
    ...
    function processImportedModules(file:SourceFile, basePath:string) {
        ...
        if (file.imports.length || file.moduleAugmentations.length) {
            ...
            for (let i = 0; i < moduleNames.length; i++) {
                ...
                const shouldAddFile = resolution && !options.noResolve &&
