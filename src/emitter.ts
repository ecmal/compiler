...
import {forEach, hasProperty, Debug, contains, filter, lastOrUndefined, Comparison, isEmpty} from "./core";
import {
    getLineStarts,
    skipTrivia,
    tokenToString,
...
import {createSourceMapWriter, getNullSourceMapWriter, SourceMapWriter} from "./sourcemap";
import {forEachChild} from "./parser";
import {writeDeclarationFile} from "./declarationEmitter";
...
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    ...
    function createFileEmitter():(jsFilePath:string, sourceMapFilePath:string, sourceFiles:SourceFile[], isBundledEmit:boolean) => void {
        ...
        function isUniqueName(name:string):boolean {
            return !resolver.hasGlobalName(name) && !hasProperty(currentFileIdentifiers, name) && !hasProperty(generatedNameSet, name);
        }
            ...
            if (!nodeIsSynthesized(node) && node.parent.kind !== SyntaxKind.ArrowFunction) {
                if (node.expression.kind === SyntaxKind.TypeAssertionExpression || node.expression.kind === SyntaxKind.AsExpression) {
                    ...
                    if (operand.kind !== SyntaxKind.PrefixUnaryExpression &&
                        ...
                        operand.kind !== SyntaxKind.NewExpression && !(operand.kind === SyntaxKind.CallExpression && node.parent.kind === SyntaxKind.NewExpression) && !(operand.kind === SyntaxKind.FunctionExpression && node.parent.kind === SyntaxKind.CallExpression) && !(operand.kind === SyntaxKind.NumericLiteral && node.parent.kind === SyntaxKind.PropertyAccessExpression)) {
                        emit(operand);
                        return;
                    }
                }
        ...
        function emitConvertedLoopCall(loop:ConvertedLoop, emitAsBlock:boolean):void {
            ...
            const isSimpleLoop =
                !loop.state.nonLocalJumps && !loop.state.labeledNonLocalBreaks && !loop.state.labeledNonLocalContinues;
            ...
            if (node) {
                ...
                for (const parameter of node.parameters) {
                    if (nodeIsDecorated(parameter)) {
                        ...
                        argumentsWritten += emitList(decorators, 0, decorators.length, /*multiLine*/ true, /*trailingComma*/ false, /*leadingComma*/ leadingComma, /*noTrailingNewLine*/ true, (decorator:Decorator) => {
