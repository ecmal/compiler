...
namespace ts {
    ...
    export function emitFiles(resolver: EmitResolver, host: EmitHost, targetSourceFile: SourceFile): EmitResult {
        ...
        const extendsHelper = `
__.extendClass = __.extendClass || function (d, b) {
    if(!b) return;
    ...
    d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
        ...
        const assignHelper = `
__.assign = __.assign || Object.assign || function(t) {
...
__decorate = __decorate || function (decorators, target, key, desc) {
    if(typeof decorators =='function'){decorators = decorators()}
...
__metadata = __metadata || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};`;

        // emit output for the __param helper function
        const paramHelper = `
__param = __param || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};`;

        const awaiterHelper = `
__.awaiter = __.awaiter || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        ...
        function createFileEmitter(): (jsFilePath: string, sourceMapFilePath: string, sourceFiles: SourceFile[], isBundledEmit: boolean) => void {
            ...
            let classEmitted: boolean;
            let helpersEmitted: boolean;
            let decorateEmitted: boolean;
            let insideSystemModule:boolean;
            ...
            function emitExportMemberAssignment(node: FunctionLikeDeclaration | ClassDeclaration,noline=false) {
            ...
            function emitAsyncFunctionBodyForES6(node: FunctionLikeDeclaration) {
                ...
                // `this` and `arguments` objects to `__.awaiter`. The generator function
                // passed to `__.awaiter` is executed inside of the callback to the
                // promise constructor.
                //
                ...
                //
                //  // output
                //  let a = (b) => __.awaiter(this, void 0, void 0, function* () {
                //      yield b;
                //  });
                //
                ...
                //
                //  // output
                //  let a = (b) => __.awaiter(this, arguments, void 0, function* (arguments) {
                //      yield arguments[0];
                //  });
                //
                ...
                //  // output
                //  let a = function (b) {
                //      return __.awaiter(this, void 0, void 0, function* () {
                //          yield b;
                //      });
                //  }
                //
                ...
                //  // output
                //  let a = function (b) {
                //      return __.awaiter(this, arguments, void 0, function* (_arguments) {
                //          yield _arguments[0];
                //      });
                //  }
                //
                ...
                //  // output
                //  let a = function (b) {
                //      return __.awaiter(this, arguments, MyPromise, function* (_arguments) {
                //          yield _arguments[0];
                //      });
                //  }
                //
                ...
                if (!isArrowFunction) {
                    ...
                    if (resolver.getNodeCheckFlags(node) & NodeCheckFlags.AsyncMethodWithSuperBinding) {
                        writeLines(`
const __super = (function (geti, seti) {
    ...
    return name => cache[name] || (cache[name] = { get value() { return geti(name); }, set value(v) { seti(name, v); } });
                        ...
                        write(`const __super = name => super[name];`);
                        writeLine();
                    }
            ...
            function emitSignatureAndBody(node: FunctionLikeDeclaration) {
                ...
                if(modulekind == ModuleKind.System && node.parent == currentSourceFile && insideSystemModule){
                    writeLine();
                    write("module.define('function', ")
                    emitDeclarationName(node);
                    write(")")
                    writeLine();
                }
            ...
            function emitClassLikeDeclarationBelowES6(node: ClassLikeDeclaration) {
                ...
                computedPropertyNamesToGeneratedNames = undefined;
                increaseIndent();
                if (baseTypeNode && modulekind != ModuleKind.System) {
                    writeLine();
                    emitStart(baseTypeNode);
                    write("__.extendClass(");
                    emitDeclarationName(node);
                    write(", __super);");
                ...
                // static initializer
                var staticInitProperties = getInitializedProperties(node, /*isStatic*/ true);
                if(baseTypeNode || (staticInitProperties && staticInitProperties.length)){
                    if(modulekind == ModuleKind.System && node.parent == currentSourceFile){
                        emitDeclarationName(node);
                        write(".__initializer = function(__parent){")
                        increaseIndent();
                        writeLine();
                        write("__super=__parent;")
                        writeLine();
            ...
            function isClassDecorated(node:ClassDeclaration):boolean{
                ...
                if(nodeCanBeDecorated(node)){
                    ...
                    if(node.members && node.members.length){
                        node.members.forEach((member:ClassElement)=>{
                            isDecorated = isClassMemberDecorated(member) || isDecorated;
                        })
                    }
                }
                if(isDecorated){
                    node.flags |= NodeFlags.HasDecorators;
                }
                return isDecorated;
            }
            function isClassMemberDecorated(node:ClassElement):boolean{
                var isDecorated = false;
                if(nodeCanBeDecorated(node)) {
                    if (node.decorators && node.decorators.length) {
                        isDecorated=true;
                    }
                    let callable: FunctionLikeDeclaration;
                    if(node.kind==SyntaxKind.SetAccessor){
                        callable = <ConstructorDeclaration>node;
                    }else
                    if(node.kind==SyntaxKind.SetAccessor){
                        callable = <SetAccessorDeclaration>node;
                    }else
                    if (node.kind === SyntaxKind.MethodDeclaration) {
                        callable = <MethodDeclaration>node;
                    }
                    if(callable && callable.parameters){
                        callable.parameters.forEach((parameter:ParameterDeclaration)=>{
                            isDecorated = isClassMemberParameterDecorated(parameter) || isDecorated;
                        })
                    }
                }
                if(isDecorated){
            ...
            function isClassMemberParameterDecorated(node:ParameterDeclaration):boolean{
                if (node.decorators && node.decorators.length) {
                    node.flags |= NodeFlags.HasDecorators;
                    return true;
                }else{
            ...
            function emitDecoratorsOfClass(node: ClassLikeDeclaration, decoratedClassAlias: string) {
                if(node.kind == SyntaxKind.ClassDeclaration && isClassDecorated(<ClassDeclaration>node)){
                    emitDeclarationName(node);
            ...
            function emitDecoratorsOfConstructor(node: ClassLikeDeclaration, decoratedClassAlias: string) {
                ...
                if(decorators && decorators.length){
                    ...
                    increaseIndent();
                    emitList(
                        decorators, 0, decorators.length,
                        /*multiLine*/ true,
                        /*trailingComma*/ false,
                        /*leadingComma*/ false,
                        /*noTrailingNewLine*/ true,
                        decorator => emitDecorator(decorator)
                    )
                    decreaseIndent();
                    writeLine();
                    emitEnd(decorators);
            ...
            function emitDecoratorsOfMembers(node: ClassLikeDeclaration, staticFlag: NodeFlags) {
                for (const member of node.members) {
                    if ((member.flags & NodeFlags.Static) !== staticFlag || !nodeCanBeDecorated(member)) {
                        continue;
                    }

                    let parentHasDecorators = node.decorators && node.decorators.length;
                    let memberHasDecorators = member.decorators && member.decorators.length;
                    let memberIsDecorated = (member.flags & NodeFlags.HasDecorators)==NodeFlags.HasDecorators;
                    if(!(parentHasDecorators||memberHasDecorators||memberIsDecorated)){
                        continue;
                    }
                    // skip an accessor declaration if it is not the first accessor
                    let decorators: NodeArray<Decorator> = member.decorators;
                    let callable: FunctionLikeDeclaration;
                    if(member.kind == SyntaxKind.SetAccessor){
                        callable = <SetAccessorDeclaration>member;
                    }else
                    if (member.kind === SyntaxKind.MethodDeclaration) {
                        callable = <MethodDeclaration>member;
                    }
                    writeLine();
                    write("__decorate(")
                    emitDeclarationKind(member);
                    write(",")
                    emitExpressionForPropertyName(member.name);
                    write(",");
                    write(String(getDecoratedMemberFlags(member.flags)))
                    write(",");
                    if(memberHasDecorators || parentHasDecorators){
                        emitSerializedTypeOfNode(member);
                    }else{
                        write("null")
                    }
                    write(",")
                    if(callable && (memberHasDecorators || parentHasDecorators)){
                        emitSerializedReturnTypeOfNode(member);
                    }else{
                        write("null")
                    }
                    write(",")
                    if(memberHasDecorators){
                        write("[");
                        emitStart(decorators);
                        writeLine();
                        increaseIndent();
                    ...
                    write(",")
                    if(callable && callable.parameters && callable.parameters.length){
                        write('[');
                        writeLine();
                        increaseIndent();
                        emitParametersOfMethod(callable);
                        decreaseIndent();
                        writeLine();
                        write(']');
                    }else{
            ...
            function emitParametersOfMethod(node: FunctionLikeDeclaration){
                if (node && node.parameters && node.parameters.length) {
                    node.parameters.forEach((parameter:ParameterDeclaration,index:number,arr:ParameterDeclaration[])=>{
                        write('["');
                        emitIdentifier(<Identifier>parameter.name);
                        write('",')
                        var flags = 0;
                        ...
                        if (nodeIsDecorated(parameter)) {
                            ...
                            writeLine();
                            increaseIndent();
                            emitList(parameter.decorators, 0, parameter.decorators.length,
                                /*multiLine*/ true,
                                /*trailingComma*/ false,
                                /*leadingComma*/ false,
                                /*noTrailingNewLine*/ true,
                                decorator => {
                                    emitDecorator(decorator);
                                });
                            decreaseIndent();
                            writeLine();
                            write(']');
            ...
            function emitDecorator(decor:Decorator){
                emitStart(decor);
                if(decor.expression.kind == SyntaxKind.CallExpression){
                    var callExp:CallExpression = <CallExpression>decor.expression;
                    write('[');
                    emit(callExp.expression);
                ...
                if(decor.expression.kind == SyntaxKind.Identifier){
                    var idExp:Identifier = <Identifier>decor.expression;
                    write('[');
                    emit(idExp);
                    write(']');
                }
            ...
            function getDecoratedMemberFlags(flags:number):number{
                var mf = 0;
                mf |= (((flags & NodeFlags.Static) == NodeFlags.Static)                  ? 1   : 0);
                mf |= (((flags & NodeFlags.Public) == NodeFlags.Public)                  ? 2   : 0);
                mf |= (((flags & NodeFlags.Protected) == NodeFlags.Protected)            ? 4   : 0);
                mf |= (((flags & NodeFlags.Private) == NodeFlags.Private)                ? 8   : 0);
                mf |= (((flags & NodeFlags.HasDecorators) == NodeFlags.HasDecorators)    ? 16  : 0);
                mf |= (((flags & NodeFlags.Abstract) == NodeFlags.Abstract)              ? 32  : 0);
                mf |= (((flags & NodeFlags.Export) == NodeFlags.Export)                  ? 64  : 0);
                mf |= (((flags & NodeFlags.Default) == NodeFlags.Default)                ? 128 : 0);
                return mf;
            }
                ...
                if (node) {
                    switch (node.kind) {
                        ...
                        case SyntaxKind.ArrayType:
                            write("__type(Array,");
                            emitSerializedTypeNode((<ArrayTypeNode>node).elementType)
                            write(")")
                            return;
                        case SyntaxKind.TupleType:
                            var aNode = <TupleTypeNode>node;
                            write("__type(Array,[");
                            aNode.elementTypes.forEach((t,i)=>{
                                if(i>0){
                                    write(",")
                                }
                        ...
                        case SyntaxKind.TypeReference:
                            var tRef = <TypeReferenceNode>node;
                            if(tRef.typeArguments && tRef.typeArguments.length) {
                                write("__type(");
                                emitSerializedTypeReferenceNode(tRef);
                                tRef.typeArguments.forEach((t, i)=> {
                                    write(",")
                                    emitSerializedTypeNode(t);
                                })
                                write(")");
            ...
            function emitSerializedTypeReferenceNode(node: TypeReferenceNode) {
                ...
                switch (result) {
                    ...
                    case TypeReferenceSerializationKind.ObjectType:
                        const resolvedType:Type = resolver.getTypeReferenceSerializationType(typeName);
                        if(resolvedType){
                            emitEntityNameAsExpression(typeName, /*useFallback*/ false);
                        }else{
                            write('Object');
                    ...
                    if (valueDeclaration) {
                        ...
                        if (parameterCount > 0) {
                            for (let i = 0; i < parameterCount; i++) {
                                ...
                                if(parameters[i].name.kind==SyntaxKind.Identifier){
                                    write('"')
                                    emitIdentifier(<Identifier>parameters[i].name)
                                    write('",')
                                }
                                ...
                                if (parameters[i].dotDotDotToken) {
                                    ...
                                    if (parameterType.kind === SyntaxKind.TypeReference && (<TypeReferenceNode>parameterType).typeArguments && (<TypeReferenceNode>parameterType).typeArguments.length === 1) {
                                        parameterType = (<TypeReferenceNode>parameterType).typeArguments[0];
                                    }
                                    else {
                                        parameterType = undefined;
            ...
            function emitDeclarationKind(node:Node){
                write(String(node.kind))
            }
            function emitDeclarationType(node:Node){
                write('"');
                write(getNodeKind(node))
                write('"');
            }
            function getNodeKind(node:Node):string{
                return <string>((<any>ts).SyntaxKind[node.kind]);
            }
            ...
            function emitSerializedTypeMetadataOld(node: Declaration, writeComma: boolean): number {
            ...
            function processTopLevelVariableAndFunctionDeclarations(node: SourceFile): (Identifier | Declaration)[] {
                ...
                let hoistedClassDeclarations: ClassDeclaration[];
                let hoistedInterfaceDeclarations: InterfaceDeclaration[];
                let hoistedEnumDeclarations: EnumDeclaration[];
                ...
                if (hoistedEnumDeclarations) {
                    for (const f of hoistedEnumDeclarations) {
                        writeLine();
                        emit(f);
                        if (f.flags & NodeFlags.Export) {
                            if (!exportedDeclarations) {
                ...
                if (hoistedClassDeclarations) {
                    for (const f of hoistedClassDeclarations) {
                        writeLine();
                        emit(f);
                        if (f.flags & NodeFlags.Export) {
                            if (!exportedDeclarations) {
                ...
                function visit(node: Node): void {
                    ...
                    if (node.kind === SyntaxKind.ClassDeclaration) {
                        if (!hoistedClassDeclarations) {
                            hoistedClassDeclarations = [];
                        }
                        hoistedClassDeclarations.push(<ClassDeclaration>node);
                        return;
                    }
                    if (node.kind === SyntaxKind.InterfaceDeclaration) {
                        if (!hoistedInterfaceDeclarations) {
                            hoistedInterfaceDeclarations = [];
                        }
                        hoistedInterfaceDeclarations.push(<InterfaceDeclaration>node);
                        return;
                    }
                    if (node.kind === SyntaxKind.EnumDeclaration) {
                        if (!hoistedEnumDeclarations) {
                            hoistedEnumDeclarations = [];
                        }
                        hoistedEnumDeclarations.push(<EnumDeclaration>node);
                        return;
                    }
            ...
            function emitClassInitialization(node:ClassDeclaration){
               const baseTypeNode = getClassExtendsHeritageClauseElement(node);
                writeLine();
                emitStart(node);
                emitDeclarationName(node);
                ...
                for (let i = startIndex; i < node.statements.length; i++) {
                    ...
                    switch (statement.kind) {
                        ...
                        case SyntaxKind.ImportDeclaration:
                            continue;
                        case SyntaxKind.ClassDeclaration:
                            emitClassInitialization(<ClassDeclaration>statement)
                            continue;
                        case SyntaxKind.ExportDeclaration:
            ...
            function emitSystemModule(node: SourceFile,  emitRelativePathAsModuleName?: boolean): void {
                ...
                exportFunctionForFile = "module.export";
                exportFunctionForFile = "module.export";
                contextObjectForFile = "module.context";
                writeLine();
    ...
    if (typeof module === 'object' && typeof module.exports === 'object') {
        ...
        define(`);
            ...
            function emitEmitSystemHelpers(node: SourceFile): void {
                helpersEmitted = true;
                write(`var __ = this;`);
            }
            function emitEmitHelpers(node: SourceFile): void {
                // Only emit helpers if the user did not say otherwise.
                if (!compilerOptions.noEmitHelpers && modulekind!=ModuleKind.System) {
                    if(!helpersEmitted){
                        writeLines(`var __ = (this && this.__helpers)||{}`);
                        helpersEmitted=true;
                    }
