...
namespace ts {
    ...
    class NodeObject implements Node {
        ...
        public getKind(){
            return (<any>ts).SyntaxKind[this.kind];
        }
        public toJSON():any{
            var node:any = {
                kind     : this.getKind(),
                flags    : this.flags,
                children : this._children?this._children.map(c=>{
                    return (<NodeObject> c).toJSON()
                }):void 0
            };
            var self:any = <any> this;
            for(var i in self){
                if((typeof self[i] =='object' && ((self[i] instanceof NodeObject) ||Array.isArray(self[i]))) && i!='pos' && i!='parent' && i!='end' && i!='_children' && i!='kind' && i!='symbol'){
                    node[i] = self[i];
                }
            }
            return node;
